import 'package:admin_app/DetailedPendingOrders.dart';
...
class _PendingOrdersState extends State<PendingOrders> {
  CollectionReference Orders=FirebaseFirestore.instance.collection('Orders').doc('Pending').collection('Pending');
  @override
  Widget build(BuildContext context) {
    return StreamBuilder(
        ...
        (BuildContext context,AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text('Currently Pending Books: ${snapshot.data.docs.length}'),
              backgroundColor: Colors.blue,
            ),
            body: snapshot.data.docs.length==0?Center(child: Text('Your do not have any Orders!!!', style: TextStyle(fontSize: 24.0),)):ListView.builder(
                              ...
                              child: InkWell(
                                onTap: (){
                                  Navigator.push(context,
                                      MaterialPageRoute(builder: (context) => DetailedPendingOrders(
